...
var links =[{"title":"帮助中心","content":["账户管理","购物指南","订单操作"]},
...
var sites = ["小米商城","MIUI","米家","米聊","多看","路由器","米粉卡","小米天猫店","隐私政策","问题反馈","Select Region"];
var infoLinks=["1.png","truste.png","v-logo-1.png","v-logo-2.png","v-logo-3.png"];
export default class Footer extends React.Component{
	render(){
		var linkItem =links.map((item,index)=>{
			var p = item.content.map((it,i)=>{
				return(
								...
								<p>
									{
										sites.map((item,index)=>{
											...
											return(
												<a key={index} className="info-text-a">{item}{sep}</a>
											)
										})
									}
								</p>
